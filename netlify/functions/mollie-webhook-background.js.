  ...
  try {
    ...
    if (plan === 'subscription') {
      ...
      if (!customerId) {
        ...
        // Sla customer → sessionId lookup op in Firebase
        await dbSet(`customerIndex/${customerId}`, sessionId);
      }

      // Recurring subscription aanmaken
      ...
      await dbSet(`sessions/${sessionId}`, {
        ...
        mollieManateId: payment.mandateId || null,
    ...
    if (plan === 'subscription_recurring') {
      ...
      // Zoek sessionId op via customerIndex in Firebase
